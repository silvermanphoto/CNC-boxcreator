# Box DXF CNC Layout Importer — v131
#
# Imports DXF files from the Box SVG-to-DXF conversion into Fusion 360
...
def make_log_path():
    ...
    return os.path.join(desktop, f"Box_DXF_Import_{ts}.txt")
...
def run(context):
    ...
    try:
        log(LOG, "=" * 60)
        log(LOG, "Box DXF CNC Layout Importer v131")
        ...
        if ui:
            msg = (
                f"Box DXF CNC Layout imported successfully!\n\n"
            ...
            ui.messageBox(msg, "Box CNC Layout v131")
        ...
        if ui:
            ui.messageBox(
                f"Error importing Box DXFs:\n\n{e}\n\nSee log: {LOG}",
                "Box CNC Layout — Error"
            )
