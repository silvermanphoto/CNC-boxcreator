#!/usr/bin/env python3
"""
# CNC GENERATOR - CARBIDE-OPTIMIZED v1.09
# ========================================
# PRODUCTION RELEASE v1.09
#
# Key Changes:
...
def generate_master_carbide_layout(version, f_front, f_back, f_top, f_bot, f_left, f_right, geom_back, cleat_data):
    ...
    # 3. GENERATE MASTER SVG (COMBINED SHEETS)
    
    # Calculate Total Canvas Size
    # We place sheets side-by-side with a gap.
    SHEET_GAP = 4.0 # 4 inches between artboards
    
    total_canvas_w = 0
    max_canvas_h = 0
    
    sheet_offsets = [] # List of x_offsets
    ...
    # Helper to clean/prepare SVG content 
    ...
    # Iterate Sheets and Items
    for s_idx, sheet in enumerate(packer.sheets):
        sheet_x_offset = sheet_offsets[s_idx]
        
        # Draw Artboard Border (Optional, helpful for visual separation)
        # We put it in annotations or a dedicated guide layer? 
        # Using ANNOTATIONS for now or just skip. 
        # Let's add a "Sheet Frame" rect in Annotations.
        layers['00_ANNOTATIONS'].append(f'<rect x="{f(sheet_x_offset)}" y="0" width="{f(sheet["w"])}" height="{f(sheet["h"])}" fill="none" stroke="#cccccc" stroke-width="0.05" />')
        # Font Size 150pt ~= 2.1in
        layers['00_ANNOTATIONS'].append(create_text(sheet_x_offset + sheet['w']/2, -1.0, f"Sheet {s_idx+1} ({sheet['w']}x{sheet['h']})", 2.1, "#000000"))
        ...
        for p_info in sheet['items']:
            ...
            # Global Coordinates
            global_x = sheet_x_offset + local_x
            global_y = local_y
            
            # Transform for Geometry
            # translate(GX, GY) [rotate?]
            if rotated:
                 # If rotated, top-left of visual bbox is (GX, GY).
                 # Transform: translate(GX + H_orig, GY) rotate(90)
                 tf = f'translate({f(global_x + part["h"])}, {f(global_y)}) rotate(90)'
                 # Bounding Box for Label: (GlobalX, GlobalY, PartH, PartW)
                 lbl_w = part['h']
                 lbl_h = part['w']
            else:
                 tf = f'translate({f(global_x)}, {f(global_y)})'
                 lbl_w = part['w']
                 lbl_h = part['h']
            ...
            # Identify active layers for this part for annotation
            active_layers = set()
            ...
            # Part Name (150pt ~= 2.1in)
            part_name = part['id'].replace("_", " ")
            layers['00_ANNOTATIONS'].append(create_text(cx, cy - 0.2, part_name, 2.1, "#000000"))
            
            # Layer Details (100pt ~= 1.4in)
            # Map codes to "Friendly" names
            layer_map = {
                '01_CUTS': 'CONTOUR (Outside)',
                '02_HOLES': 'HOLES (Inside)',
                '03_RABBETS': 'POCKET', # Usually rabbets are pockets
                '04_POCKETS': 'POCKET',
                '05_WINDOWS': 'CONTOUR (Inside)',
                '06_SCORES': 'NO OFFSET (Score)'
            }
            
            # Build Description
            details = []
            for al in sorted(active_layers):
                if al in layer_map:
                    details.append(layer_map[al])
            
            # Dedup details
            details = sorted(list(set(details)))
            
            # Lines of text
            line_spacing = 1.5 # inches
            start_y = cy
            
            for i, d in enumerate(details):
                layers['00_ANNOTATIONS'].append(create_text(cx, start_y + ((i+1)*line_spacing), d, 1.4, "#333333"))

    # Output All Layers
    for l_name in sorted(layers.keys()):
        content_blocks = layers[l_name]
        if content_blocks:
    ...
    # Return Dict with Single Combined File
...
class CarbideOptimizedApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        # Version
        tk.Label(container, text="v1.09", font=FONT_VERSION, bg=BG_COLOR, fg=TEXT_HINT).place(relx=1.0, rely=1.0, anchor="se")
