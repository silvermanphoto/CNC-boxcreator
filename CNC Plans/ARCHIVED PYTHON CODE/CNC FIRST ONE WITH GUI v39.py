#!/usr/bin/env python3
"""
Box Parametric CNC Shadowbox Generator v39
=============================================
GUI Version
"""
...
class ShadowboxApp(tk.Tk):
    ...
    def run_generation(self):
        try:
            ...
            vers = [int(re.search(r'v(\d+)', c.name).group(1)) for c in out.iterdir() if "Box SVGs v" in c.name and re.search(r'v(\d+)', c.name)]
            ver = max(vers) + 1 if vers else 1
            fd = out / f"Box SVGs v{ver}"; fd.mkdir(parents=True, exist_ok=True)
