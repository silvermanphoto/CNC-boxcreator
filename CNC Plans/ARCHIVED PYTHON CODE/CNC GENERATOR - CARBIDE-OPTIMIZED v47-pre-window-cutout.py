...
class BoxPreview3D(tk.Frame):
    ...
    def __init__(self, parent, width=280, height=280):
        ...
        # Initial empty box
        self.update_box(10, 10, 4, 0.59)  # Default dimensions in inches
    ...
    def update_box(self, width_in, height_in, depth_in, stock_thk_in):
        ...
        for verts, name in panels:
            color = self.PANEL_COLORS[name]
...
class CarbideOptimizedApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        # 3D Preview label
        tk.Label(right_col, text="PREVIEW", font=FONT_SECTION, bg=BG_COLOR, fg=ACCENT_COLOR).pack(anchor="w", pady=(0, 6))

        # 3D Preview widget
        self.preview_3d = BoxPreview3D(right_col, width=280, height=240)
        self.preview_3d.pack(pady=(0, 15))

        # Hint text for interaction
        tk.Label(right_col, text="Drag to rotate", font=FONT_HINT, bg=BG_COLOR, fg=TEXT_HINT).pack(anchor="center", pady=(0, 10))

        # Generate button - Kermit green with white text
        KERMIT_GREEN = "#5DB761"
        KERMIT_HOVER = "#4CA750"
        gen_btn = tk.Button(right_col, text="GENERATE SVGs", command=self.run_generation,
                            font=("Lato", 14, "bold"), bg=KERMIT_GREEN, fg="white",
                            relief=tk.RAISED, bd=2, padx=30, pady=14, cursor="hand2",
                            activebackground=KERMIT_HOVER, activeforeground="white",
                            highlightthickness=0)
        gen_btn.pack(pady=(5, 0))

        # Add hover effect
        def on_enter(e):
            gen_btn.config(bg=KERMIT_HOVER)
        def on_leave(e):
            gen_btn.config(bg=KERMIT_GREEN)
        gen_btn.bind("<Enter>", on_enter)
        gen_btn.bind("<Leave>", on_leave)
    ...
    def _setup_live_preview_updates(self):
        ...
        def on_change(*args):
            try:
                ...
                self.preview_3d.update_box(width, height, depth, stock_in)
            except (ValueError, AttributeError):
                pass  # Ignore invalid input during typing

        # Trace changes on dimension variables
        for var_name in ['width', 'height', 'depth', 'stock_thk']:
