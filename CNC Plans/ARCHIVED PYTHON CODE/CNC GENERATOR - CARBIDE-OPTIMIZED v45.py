...
def generate_master_carbide_layout(version, f_front, f_back, f_top, f_bot, f_left, f_right):
    ...
    REFINED LAYOUT (Landscape):
      ...
      - Text labels placed safely above parts.
    """
    GAP = 2.0        # Uniform gap between parts and edges
    TEXT_H = 0.8     # Reserve space for text label + buffer
    ...
    # Column 3: Rail Stack
    # Rails are horizontal strips.
    # Top/Bot: Width = total_w
    # Left/Right: Width = total_h (Normally vertical, but rotated 90deg for grain direction)
    # However, generate_rail_parts already generates them as horizontal strips if we asked for it?
    # Wait, in generate_rail_parts:
    #   Horizontal (Top/Bot): rail_w = Total_Width - 2*Stock. Height = Depth.
    #   Vertical (Side): rail_w = Total_Height. Height = Depth. (This is already "Long dimension along X")
    #   So "Height" of all rails in SVG space is `box_depth`.
    
    rail_w_max = max(total_w, total_h)
    # Stack height: 4 rails * depth + 3 internal gaps
    ...
    # Note: We need extra room for text labels ABOVE each part?
    # Yes, let's treat the "Part Height" as actual geometry height.
    # We will offset Y by TEXT_H when placing.
    
    # Actually, let's include TEXT_H in the "effective height" of the column for centering purposes?
    # Or just align tops? Let's align TOPS of the columns for simplicity, or Center?
    # "Equidistant from edges" implies centering or uniform grid.
    
    # Let's align the START of the geometry (below the text) to be visually pleasing.
    # Simplified: Top Align the columns, starting at Y = GAP + TEXT_H.
    
    max_h = max(c1_h, c2_h, c3_h)
    canvas_h = GAP + TEXT_H + max_h + GAP
    
    # --- 2. POSITIONING ---
    
    # Y Position for top-row parts (Front, Back, Top Rail)
    # All start at same Y level
    row_y = GAP + TEXT_H 
    ...
    # Rail Sub-positions (relative to x_rails, row_y)
    # Stack: Top, Bot, Left, Right
    pos_top = (x_rails, row_y)
    pos_bot = (x_rails, row_y + box_depth + GAP + TEXT_H) 
    # Wait, need text buffer for subsequent rails too! 
    # Recalculate rail stack height to include text buffers
    
    # Rail Stack:
    # 1. Top Rail (Text + Depth)
    # 2. GAP
    # 3. Bot Rail (Text + Depth) ...
    
    rail_stack_h_refined = (4 * box_depth) + (4 * TEXT_H) + (3 * GAP)
    
    # Re-evaluate Canvas Height
    max_h_refined = max(c1_h + TEXT_H, c2_h + TEXT_H, rail_stack_h_refined)
    canvas_h = GAP + max_h_refined + GAP
    
    # Rail Y positions
    y_r1 = GAP + TEXT_H # Top
    y_r2 = y_r1 + box_depth + GAP + TEXT_H # Bot
    y_r3 = y_r2 + box_depth + GAP + TEXT_H # Left
    y_r4 = y_r3 + box_depth + GAP + TEXT_H # Right
    ...
    # Legend (Top Left usually, but might overlap if tight. Put in top margin?)
    # Let's put legend in the first GAP area (0,0 to total_w, GAP)
    # Or just overlay it safely.
    lines.append(f'  <g transform="translate({f(GAP)}, {f(GAP/2)})">')
    lines.append(f'    <text x="0" y="0" font-family="Arial" font-size="0.4" fill="black">MASTER KEY: '
                 f'<tspan fill="{COLOR_PERIMETER}">RED=CUTS</tspan> '
                 f'<tspan fill="{COLOR_WINDOW}">PURPLE=WINDOW</tspan> '
                 f'<tspan fill="{COLOR_POCKETS}">GREEN=POCKETS</tspan> '
                 f'<tspan fill="{COLOR_HOLES}">BLUE=HOLES</tspan> '
                 f'<tspan fill="{COLOR_RABBETS}">ORANGE=RABBETS</tspan>'
                 f'</text>')
    lines.append(f'  </g>')
    ...
    # Text Layer (Always on top or separate? Put in cuts for now)
    text_items = []

    for name, files_dict, (pos_x, pos_y) in layout_map:
        # Part Label (Above the part)
        # pos_y is the TOP of the part geometry. Text goes above that.
        text_y = pos_y - 0.2
        text_items.append(f'    <text x="{f(pos_x)}" y="{f(text_y)}" font-family="Arial" font-size="0.5" fill="black" stroke="none">{name}</text>')
        ...
        for fname, content in files_dict.items():
            ...
            if body_match:
                ...
                # Shift by global MARGIN_INCHES (2.0) to reset origin, then placing at pos_x, pos_y
                block = []
                block.append(f'    <g id="{name}_{layer_key}" transform="translate({f(pos_x)}, {f(pos_y)})">')
    ...
    for layer_name in sorted(layers.keys()):
        ...
        if content_blocks:
            ...
            # Add text labels to CUTS layer? Or a separate text layer?
            # Let's add text separately at the end to ensure visibility, or in CUTS.
            # User asked "Text is not overlaying".
            lines.extend(content_blocks)
            lines.append('  </g>')
    
    # Add Labels Group
    lines.append('  <g id="00_LABELS">')
    lines.extend(text_items)
    lines.append('  </g>')
...
class CarbideOptimizedApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        tk.Label(input_frame, text="Window Size (Centered)", font=("Arial", 12, "bold"), fg="#8eaaff", bg="#1e1e24").grid(row=r, column=0, columnspan=3, sticky="w", pady=(8, 3)); r += 1
        self.make_row(input_frame, r, "Window Width:", self.vars['window_w'], "inches (Default: Total - 3\")"); r += 1
        self.make_row(input_frame, r, "Window Height:", self.vars['window_h'], "inches (Default: Total - 3\")"); r += 1
