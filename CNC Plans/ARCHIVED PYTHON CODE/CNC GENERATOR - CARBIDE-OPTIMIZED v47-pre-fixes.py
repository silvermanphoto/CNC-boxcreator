#!/usr/bin/env python3
"""
CNC GENERATOR - CARBIDE-OPTIMIZED v47
======================================
Fork of v46, with clarified parameter naming.

Key Changes:
- NAMING: Renamed rabbet parameters for clarity:
  - "Front/Back Rabbet Width" → "Front/Back Rim Width" (edge to inner rabbet line)
  - "Front/Back Rabbet Depth" → "Front/Back Rabbet Cut Depth" (Z-depth of cut)
- TOOLTIPS: Added mouseover hints to explain each parameter.
...
def ensure_dependencies():
    ...
    if missing:
        ...
        try:
            ...
            # If that fails (PEP 668 environments), try with --break-system-packages
...
class BoxPreview3D(tk.Frame):
    ...
    PANEL_COLORS = {
        'front': '#49bcf6',   # Light blue (perimeter)
        'back': '#49bcf6',
        'left': '#e498c3',    # Pink (like rabbets)
        'right': '#e498c3',
        'top': '#e8cf6c',     # Yellow/gold
        'bottom': '#e8cf6c',
    }
    ...
    def update_box(self, width_in, height_in, depth_in, stock_thk_in, window_w=0, window_h=0):
        ...
        # Convert to consistent units (use inches directly)
        w, h, d = width_in, height_in, depth_in
        t = stock_thk_in  # Stock thickness
        self.window_w = window_w
        self.window_h = window_h

        # Back panel (at z=d) - always solid
        back = self._make_panel_vertices(0, 0, d, w, h, 0, 'xy')
        poly = Poly3DCollection([back], alpha=0.7, facecolor=self.PANEL_COLORS['back'],
                                edgecolor='#333333', linewidth=0.5)
        self.ax.add_collection3d(poly)

        # Left panel (at x=0)
        left = self._make_panel_vertices(0, 0, t, 0, h, d - 2*t, 'yz')
        poly = Poly3DCollection([left], alpha=0.7, facecolor=self.PANEL_COLORS['left'],
                                edgecolor='#333333', linewidth=0.5)
        self.ax.add_collection3d(poly)

        # Right panel (at x=w)
        right = self._make_panel_vertices(w, 0, t, 0, h, d - 2*t, 'yz')
        poly = Poly3DCollection([right], alpha=0.7, facecolor=self.PANEL_COLORS['right'],
                                edgecolor='#333333', linewidth=0.5)
        self.ax.add_collection3d(poly)

        # Top panel (at y=h)
        top = self._make_panel_vertices(t, h, t, w - 2*t, 0, d - 2*t, 'xz')
        poly = Poly3DCollection([top], alpha=0.7, facecolor=self.PANEL_COLORS['top'],
                                edgecolor='#333333', linewidth=0.5)
        self.ax.add_collection3d(poly)

        # Bottom panel (at y=0)
        bottom = self._make_panel_vertices(t, 0, t, w - 2*t, 0, d - 2*t, 'xz')
        poly = Poly3DCollection([bottom], alpha=0.7, facecolor=self.PANEL_COLORS['bottom'],
                                edgecolor='#333333', linewidth=0.5)
        self.ax.add_collection3d(poly)

        # Front panel (at z=0) - with optional window cutout
        if window_w > 0 and window_h > 0 and window_w < w and window_h < h:
            # Calculate window position (centered)
            win_x = (w - window_w) / 2
            win_y = (h - window_h) / 2

            # Create frame pieces around the window (4 rectangles)
            # Bottom strip
            bottom_strip = [(0, 0, 0), (w, 0, 0), (w, win_y, 0), (0, win_y, 0)]
            # Top strip
            top_strip = [(0, win_y + window_h, 0), (w, win_y + window_h, 0), (w, h, 0), (0, h, 0)]
            # Left strip
            left_strip = [(0, win_y, 0), (win_x, win_y, 0), (win_x, win_y + window_h, 0), (0, win_y + window_h, 0)]
            # Right strip
            right_strip = [(win_x + window_w, win_y, 0), (w, win_y, 0), (w, win_y + window_h, 0), (win_x + window_w, win_y + window_h, 0)]
            ...
            # Draw white window border
            window_border = [(win_x, win_y, 0), (win_x + window_w, win_y, 0),
                           (win_x + window_w, win_y + window_h, 0), (win_x, win_y + window_h, 0), (win_x, win_y, 0)]
            wx = [p[0] for p in window_border]
            wy = [p[1] for p in window_border]
            wz = [p[2] for p in window_border]
            self.ax.plot(wx, wy, wz, color='white', linewidth=2.5)
        else:
            # No window - solid front panel
            front = self._make_panel_vertices(0, 0, 0, w, h, 0, 'xy')
            poly = Poly3DCollection([front], alpha=0.7, facecolor=self.PANEL_COLORS['front'],
                                    edgecolor='#333333', linewidth=0.5)
            self.ax.add_collection3d(poly)

        # Set equal aspect ratio
        max_dim = max(w, h, d)
        self.ax.set_xlim([-max_dim*0.1, max_dim*1.1])
        self.ax.set_ylim([-max_dim*0.1, max_dim*1.1])
        self.ax.set_zlim([-max_dim*0.1, max_dim*1.1])
...
class CarbideOptimizedApp(tk.Tk):
    def __init__(self):
        ...
        self.vars = {
            ...
            # Rabbet parameters (rim_width = distance from panel edge to inner rabbet line)
            'front_rim_width': tk.StringVar(value="0.3125"),
            'front_rabbet_cut_depth': tk.StringVar(value="0.3125"),
            'back_rim_width': tk.StringVar(value="0.3125"),
            'back_rabbet_cut_depth': tk.StringVar(value="0.3125"),
    ...
    def setup_ui(self):
        ...
        ACCENT_COLOR = "#49bcf6"     # Light blue accent
        INPUT_BG = "#ffffff"         # White input fields
        INPUT_BORDER = "#cccccc"     # Light border

        # Fonts (Lato with fallbacks)
        FONT_TITLE = ("Lato", 24, "bold")
        FONT_SECTION = ("Lato", 11, "bold")
        FONT_LABEL = ("Lato", 11)
        FONT_INPUT = ("Lato", 11)
        FONT_HINT = ("Lato", 9)
        ...
        # Store colors for make_row
        self.ui_colors = {
            'bg': BG_COLOR, 'text': TEXT_PRIMARY, 'hint': TEXT_HINT,
            'input_bg': INPUT_BG, 'accent': ACCENT_COLOR
        }
        self.ui_fonts = {
            'label': FONT_LABEL, 'input': FONT_INPUT, 'hint': FONT_HINT
        }
        ...
        # Subtle divider
        tk.Frame(container, height=1, bg="#dddddd").pack(fill=tk.X, pady=(10, 15))
        ...
        left_col.pack(side="left", fill=tk.Y, padx=(0, 40))

        # Output folder row
        folder_frame = tk.Frame(left_col, bg=BG_COLOR)
        folder_frame.pack(fill=tk.X, pady=(0, 12))
        tk.Label(folder_frame, text="Output Folder", font=FONT_LABEL, bg=BG_COLOR, fg=TEXT_PRIMARY).pack(anchor="w")
        folder_input_frame = tk.Frame(folder_frame, bg=BG_COLOR)
        folder_input_frame.pack(fill=tk.X, pady=(3, 0))
        folder_entry = tk.Entry(folder_input_frame, textvariable=self.vars['out_folder'],
                                font=FONT_INPUT, bg=INPUT_BG, fg=TEXT_PRIMARY,
        ...
        tk.Button(folder_input_frame, text="Browse", command=self.browse_folder,
                  font=("Lato", 10), bg="#e0e0e0", fg=TEXT_PRIMARY, relief=tk.FLAT,
                  padx=12, pady=2).pack(side="left", padx=(8, 0))

        # DIMENSIONS section
        tk.Label(left_col, text="DIMENSIONS", font=FONT_SECTION, bg=BG_COLOR, fg=ACCENT_COLOR).pack(anchor="w", pady=(8, 6))

        dim_grid = tk.Frame(left_col, bg=BG_COLOR)
        dim_grid.pack(fill=tk.X)

        # Row 1: Width, Height
        self.make_compact_row(dim_grid, 0, 0, "Width", self.vars['width'], "in")
        self.make_compact_row(dim_grid, 0, 3, "Height", self.vars['height'], "in")

        # Row 2: Depth, Stock
        self.make_compact_row(dim_grid, 1, 0, "Depth", self.vars['depth'], "in")
        self.make_compact_row(dim_grid, 1, 3, "Stock", self.vars['stock_thk'], "mm")

        # Row 3: Tool Dia, Tolerance
        self.make_compact_row(dim_grid, 2, 0, "Tool Dia", self.vars['tool_primary'], "in")
        self.make_compact_row(dim_grid, 2, 3, "Tolerance", self.vars['glue_gap'], "in")

        # JOINERY section
        tk.Label(left_col, text="JOINERY", font=FONT_SECTION, bg=BG_COLOR, fg=ACCENT_COLOR).pack(anchor="w", pady=(14, 6))

        join_grid = tk.Frame(left_col, bg=BG_COLOR)
        join_grid.pack(fill=tk.X)

        # Row 1: Front Rim, Back Rim
        self.make_compact_row(join_grid, 0, 0, "Front Rim", self.vars['front_rim_width'], "in",
                              tooltip="Distance from panel edge to inner rabbet line")
        self.make_compact_row(join_grid, 0, 3, "Back Rim", self.vars['back_rim_width'], "in",
                              tooltip="Distance from panel edge to inner rabbet line")

        # Row 2: Front Cut Depth, Back Cut Depth
        self.make_compact_row(join_grid, 1, 0, "Front Depth", self.vars['front_rabbet_cut_depth'], "in",
                              tooltip="Z-depth of rabbet cut")
        self.make_compact_row(join_grid, 1, 3, "Back Depth", self.vars['back_rabbet_cut_depth'], "in",
                              tooltip="Z-depth of rabbet cut")

        # Row 3: Finger Width, Pilot Hole
        self.make_compact_row(join_grid, 2, 0, "Finger Width", self.vars['finger_width'], "mm")
        self.make_compact_row(join_grid, 2, 3, "Pilot Hole", self.vars['pilot_dia'], "mm")

        # WINDOW section
        tk.Label(left_col, text="WINDOW", font=FONT_SECTION, bg=BG_COLOR, fg=ACCENT_COLOR).pack(anchor="w", pady=(14, 6))

        win_grid = tk.Frame(left_col, bg=BG_COLOR)
        win_grid.pack(fill=tk.X)

        self.make_compact_row(win_grid, 0, 0, "Width", self.vars['window_w'], "in")
        self.make_compact_row(win_grid, 0, 3, "Height", self.vars['window_h'], "in")

        # Motor pocket checkbox
        cb = tk.Checkbutton(left_col, text="Enable Motor Pocket", variable=self.vars['motor_enabled'],
                            bg=BG_COLOR, fg=TEXT_PRIMARY, selectcolor=INPUT_BG,
                            font=FONT_LABEL, activebackground=BG_COLOR)
        cb.pack(anchor="w", pady=(12, 0))

        # RIGHT COLUMN - 3D Preview and Generate button
        right_col = tk.Frame(content, bg=BG_COLOR)
        right_col.pack(side="left", fill=tk.Y, anchor="n", pady=(0, 0))

        # 3D Preview label with Reset button
        preview_header = tk.Frame(right_col, bg=BG_COLOR)
        preview_header.pack(fill=tk.X, pady=(0, 6))
        tk.Label(preview_header, text="PREVIEW", font=FONT_SECTION, bg=BG_COLOR, fg=ACCENT_COLOR).pack(side="left")

        # Pink reset button
        PINK = "#e498c3"
        reset_btn = tk.Button(preview_header, text="Reset", command=self.reset_to_defaults,
                              font=("Lato", 9), bg=PINK, fg="white",
                              relief=tk.FLAT, padx=8, pady=1, cursor="hand2",
                              activebackground="#d080b0", activeforeground="white")
        reset_btn.pack(side="right")

        # 3D Preview widget (33% larger: 280*1.33=373, 240*1.33=320)
        self.preview_3d = BoxPreview3D(right_col, width=373, height=320)
        self.preview_3d.pack(pady=(0, 10))

        # Hint text for interaction
        tk.Label(right_col, text="Drag to rotate", font=FONT_HINT, bg=BG_COLOR, fg=TEXT_HINT).pack(anchor="center", pady=(0, 8))

        # Generate button - Green pill shape (#00be03)
        gen_btn = PillButton(right_col, width=200, height=44, color="#00be03",
                             text="GENERATE SVGs", font=("Lato", 14, "bold"),
                             command=self.run_generation)
        gen_btn.pack(pady=(5, 0))

        # Bind live updates to dimension fields
        self._setup_live_preview_updates()

        # Version number at bottom right
        version_frame = tk.Frame(container, bg=BG_COLOR)
        version_frame.pack(side="bottom", anchor="e")
        tk.Label(version_frame, text="v47", font=FONT_VERSION, bg=BG_COLOR, fg=TEXT_HINT).pack()

    def make_compact_row(self, parent, row, col, label_text, var, unit, tooltip=None):
        """Create a compact label + entry + unit in a grid cell."""
        BG = self.ui_colors['bg']
        TEXT = self.ui_colors['text']
        HINT = self.ui_colors['hint']
        INPUT_BG = self.ui_colors['input_bg']

        lbl = tk.Label(parent, text=label_text, font=self.ui_fonts['label'], bg=BG, fg=TEXT)
        lbl.grid(row=row, column=col, sticky="e", padx=(0, 5), pady=4)
        if tooltip:
            ToolTip(lbl, tooltip)

        entry = tk.Entry(parent, textvariable=var, font=self.ui_fonts['input'],
                         bg=INPUT_BG, fg=TEXT, relief=tk.SOLID, bd=1, width=8)
        entry.grid(row=row, column=col+1, sticky="w", pady=4)

        tk.Label(parent, text=unit, font=self.ui_fonts['hint'], bg=BG, fg=HINT).grid(
            row=row, column=col+2, sticky="w", padx=(3, 20), pady=4)

    def make_row(self, parent, r, label_text, var, unit, tooltip=None):
        """Legacy make_row - kept for compatibility but uses new colors."""
        BG = self.ui_colors.get('bg', "#f0f0f0")
        TEXT = self.ui_colors.get('text', "#333333")
        HINT = self.ui_colors.get('hint', "#999999")
        INPUT_BG = self.ui_colors.get('input_bg', "#ffffff")

        lbl = tk.Label(parent, text=label_text, fg=TEXT, bg=BG, font=self.ui_fonts.get('label', ("Lato", 11)))
        lbl.grid(row=r, column=0, sticky="e", padx=5, pady=3)
        if tooltip:
            ToolTip(lbl, tooltip)
        tk.Entry(parent, textvariable=var, bg=INPUT_BG, fg=TEXT, width=10,
                 font=self.ui_fonts.get('input', ("Lato", 11)), relief=tk.SOLID, bd=1).grid(row=r, column=1, sticky="w")
        tk.Label(parent, text=unit, fg=HINT, bg=BG, font=self.ui_fonts.get('hint', ("Lato", 9))).grid(row=r, column=2, sticky="w")
    ...
    def _setup_live_preview_updates(self):
        ...
        def on_change(*args):
            try:
                # Get current values
                width = float(self.vars['width'].get() or 10)
                height = float(self.vars['height'].get() or 10)
                depth = float(self.vars['depth'].get() or 4)
                stock_mm = float(self.vars['stock_thk'].get() or 15)
                stock_in = stock_mm / 25.4  # Convert mm to inches

                # Get window dimensions (default to 0 if empty)
                window_w_str = self.vars['window_w'].get().strip()
                window_h_str = self.vars['window_h'].get().strip()
                window_w = float(window_w_str) if window_w_str else 0
                window_h = float(window_h_str) if window_h_str else 0

                # Update the 3D preview
                self.preview_3d.update_box(width, height, depth, stock_in, window_w, window_h)
            except (ValueError, AttributeError):
                pass  # Ignore invalid input during typing

        # Trace changes on dimension and window variables
        for var_name in ['width', 'height', 'depth', 'stock_thk', 'window_w', 'window_h']:
            self.vars[var_name].trace_add('write', on_change)
    ...
    def reset_to_defaults(self):
        ...
        if self.settings_file.exists():
            ...
            defaults = {
                ...
                'glue_gap': 0.010,
                'finger_width': 15.0,
                'front_rim_width': "0.3125",
                'front_rabbet_cut_depth': "0.3125",
                'back_rim_width': "0.3125",
                'back_rabbet_cut_depth': "0.3125",
                'pilot_dia': 5.0,
                'motor_enabled': True,
                'motor_x_val': "",
                'window_w': "",
    ...
    def run_generation(self):
        try:
            ...
            # Rim width is the distance from edge to inner rabbet line (where holes center)
            # Internally we still use RABBET_WIDTH for the plug depth calculation
            front_rim = parse_rabbet_value(v['front_rim_width'].get(), stock_thk_in)
            back_rim = parse_rabbet_value(v['back_rim_width'].get(), stock_thk_in)
            # Convert rim width to plug depth: plug_depth = stock_thickness - rim_width
            CONFIG['FRONT_RABBET_WIDTH'] = stock_thk_in - front_rim
            CONFIG['FRONT_RABBET_DEPTH'] = parse_rabbet_value(v['front_rabbet_cut_depth'].get(), stock_thk_in)
            CONFIG['BACK_RABBET_WIDTH'] = stock_thk_in - back_rim
            CONFIG['BACK_RABBET_DEPTH'] = parse_rabbet_value(v['back_rabbet_cut_depth'].get(), stock_thk_in)
            ...
            # Default Window Size if empty: Total - 3 inches (1.5" rim)
            def get_val_or_default(var, default_val):
                val = var.get().strip()
                return float(val) if val else default_val

            window_w_in = get_val_or_default(v['window_w'], total_w_in - 3.0)
            window_h_in = get_val_or_default(v['window_h'], total_h_in - 3.0)
            
            # Ensure window isn't larger than total
            window_w_in = min(window_w_in, total_w_in - 0.5)
            window_h_in = min(window_h_in, total_h_in - 0.5)
            
            CONFIG['WINDOW_WIDTH_IN'] = window_w_in
            CONFIG['WINDOW_HEIGHT_IN'] = window_h_in
            
            # Calculate Rim Width (for reference/other uses) - assumes uniform centering
            # Taking the smaller rim if aspect ratios differ, though create_front_bezel uses window directly now
            rim_w_in = (total_w_in - window_w_in) / 2
