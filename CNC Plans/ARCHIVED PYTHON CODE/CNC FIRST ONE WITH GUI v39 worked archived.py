...
def generate_combined_layout_svg(file_contents):
    ...
    for fname, content in file_contents.items():
        part_name = fname.split('_v')[0]
        tx, ty = 0, 0
        rot = 0
        cx, cy = 0, 0 # Center of rotation (center of part canvas)
        
        if "FRONT_BEZEL" in fname:
            # Sheet 1: Centered-ish
            # Target: 30", 30"
            tx, ty = get_translation(s1_x, s1_y, 30, 30, PART_CANVAS_LARGE)
            cx, cy = PART_CANVAS_LARGE/2, PART_CANVAS_LARGE/2
            
        elif "GLAZING" in fname:
            # Sheet 1: Right Side (Vertical Strips)
            # Target: 54", 30" (Shifted slightly right to 54" to ensure clearance from 50" bezel edge)
            # ROTATE 90 Degrees
            tx, ty = get_translation(s1_x, s1_y, 54, 30, PART_CANVAS_STD)
            rot = 90
            cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2

        elif "BACK_PANEL" in fname:
            # Sheet 2: Lower Center (Shifted Left to make room for side rails)
            # Sheet is 60" wide. Panel is 40" wide.
            # If centered at 25", it spans 5" to 45". Leaves 15" on right.
            # Target: 25", 38" (Pushed down to make room for top rails)
            tx, ty = get_translation(s2_x, s2_y, 25, 38, PART_CANVAS_STD)
            cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2
            
        elif "TOP_RAIL" in fname:
             # Sheet 2: Top Center
             # Target: 30", 6"
             tx, ty = get_translation(s2_x, s2_y, 30, 6, PART_CANVAS_STD)
             cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2
             
        elif "BOTTOM_RAIL" in fname:
             # Sheet 2: Below Top Rail
             # Target: 30", 12"
             tx, ty = get_translation(s2_x, s2_y, 30, 12, PART_CANVAS_STD)
             cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2
             
        elif "LEFT_RAIL" in fname:
             # Sheet 2: Right Side (Vertical)
             # Target: 50", 35"
             tx, ty = get_translation(s2_x, s2_y, 50, 35, PART_CANVAS_STD)
             cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2
             
        elif "RIGHT_RAIL" in fname:
             # Sheet 2: Far Right Side (Vertical)
             # Target: 56", 35"
             tx, ty = get_translation(s2_x, s2_y, 56, 35, PART_CANVAS_STD)
             cx, cy = PART_CANVAS_STD/2, PART_CANVAS_STD/2
...
class ShadowboxApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        input_col.grid(row=1, column=0, sticky="n")
        
        self.visual_col = tk.Frame(self.scroll_frame, bg="#1e1e24", padx=20)
        self.visual_col.grid(row=1, column=1, sticky="n")
        ...
        rb_frame = tk.Frame(self.visual_col, bg="#1e1e24")
        rb_frame.place(x=350, y=400) # Adjust coordinates as needed

        tk.Radiobutton(rb_frame, text="Combine SVGs in one file (130\"x70\" Layout)", 
    ...
    def update_joinery_image(self):
        ...
        if os.path.exists(fname):
            ...
            img.thumbnail((800, 600)) # Resize to fit width
