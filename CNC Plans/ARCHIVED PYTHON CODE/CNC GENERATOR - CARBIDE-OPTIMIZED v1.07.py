#!/usr/bin/env python3
"""
# CNC GENERATOR - CARBIDE-OPTIMIZED v1.07
# ========================================
# PRODUCTION RELEASE v1.07
#
# Key Changes:
...
def generate_master_carbide_layout(version, f_front, f_back, f_top, f_bot, f_left, f_right, geom_back, cleat_data):
    """
    Generate optimized nesting layout using BinPacker (v1.07).
    Capable of creating multiple sheets.
    """
    GAP = 0.5 # User specified: "Nothing closer than 0.5""
    
    # 1. Collect All Parts
    # We need dimensions. The `f_*` dicts contain SVGs. 
    # We rely on Config/Geometry data to know sizes before packing.
    # Parts: FRONT, BACK, RAIL_TOP, RAIL_BOT, RAIL_LEFT, RAIL_RIGHT, CLEATS(x2), HATCH_LID(?.
    
    parts_to_pack = []
    
    # Helper to clean/prepare SVG content (ensure it's grouped and 0-0 based?)
    # The SVG content from generate_* usually has a header/footer and translated inner groups.
    # We want the RAW body (or Group) that is sized to Width/Height.
    # Actually, the file contents provided in `f_front` etc are FULL SVG files.
    # We need to extract the 'active' geometry and wrap it in a <g> that we can translate/rotate.
    
    def prepare_part(name, files, w_in, h_in, extra_id=None):
        return {
            'id': name,
            'files': files, # Dict of filename -> content
            'w': w_in,
            'h': h_in,
            'extra': extra_id
        }
    ...
    # FRONT
    # Dimensions: Total W x Total H
    # HATCH LOGIC: Hatch Lid is NESTED inside Front if it fits.
    # `generate_front_bezel_parts` creates "Window" and "Rabbets".
    # `generate_back_panel_parts` creates "Hatch Lid".
    # We need to explicitly check if Hatch Lid fits in Window.
    ...
    # Rails
    # Top/Bot (W = Total_W, H = Depth) - (Actually Total_W - 2*Thk if vertical? No, simple box logic)
    # create_rail logic:
    # Horizontal (Top/Bot): W = Total_W - 2*Thk, H = Depth? Wait, logic in `generate_rail_parts`:
    #   Horizontal: rail_w = Total_W - 2*Stock.
    #   Vertical: rail_w = Total_H.
    # Let's recalc strict bounding boxes to avoid overlap.
    stock_thk = convert_to_inches(CONFIG['STOCK_THICKNESS'])
    ...
    w_sides = total_h
    # Note: Side rails are Vertical in 'generate' logic but we just need bounds.
    # `generate_rail_parts` for sides sets `rail_w_in = total_h`.
    ...
    if hatch_data:
        ...
        if is_nested:
            # Special Handling: We do NOT pack Hatch Lid separately.
            # We must attach it to FRONT.
            # But FRONT's `files` dict doesn't have Hatch Lid.
            # We need to Merge them in layout generation later.
            # We flag FRONT.
            # Find FRONT
            for p in parts_to_pack:
                if p['id'] == "FRONT":
                    p['nested_hatch'] = hatch_data
                    # Add Hatch Files to Front Files? 
                    # No, we'll keep them standard but render them relative to Front.
        else:
            # Add as independent part
            # Construct File Dict for Hatch Lid
            hatch_files = {f"HATCH_LID.v{version}.svg": hatch_data['svg_content']}
            parts_to_pack.append(prepare_part("HATCH_LID", hatch_files, lid_w, lid_h))
    ...
    for sheet_idx, sheet in enumerate(packer.sheets):
        lines = []
        lines.append(f'<?xml version="1.0" encoding="UTF-8"?>')
        lines.append(f'<svg xmlns="http://www.w3.org/2000/svg" width="{sheet["w"]}in" height="{sheet["h"]}in" viewBox="0 0 {sheet["w"]} {sheet["h"]}">')
        label = f"MASTER_LAYOUT_v{version}_SHEET_{sheet_idx+1}"
        lines.append(f'  <title>{label}</title>')
        lines.append(f'  <g id="{label}">')
        
        # Accumulate Layers
        layers = {
            '01_CUTS': [], '02_HOLES': [], '03_RABBETS': [], 
            '04_POCKETS': [], '05_WINDOWS': [], '06_SCORES': []
        }
        
        def get_layer_type(fname):
            if "PERIMETER" in fname: return '01_CUTS'
            if "HATCH_LID" in fname: return '01_CUTS'
            if "HOLES_THROUGH" in fname: return '02_HOLES'
            if "HOLES" in fname: return '02_HOLES'
            if "RABBETS" in fname: return '03_RABBETS'
            if "HOLES_CSINK" in fname: return '04_POCKETS'
        ...
        for p_info in sheet['items']:
            ...
            # Rotation Transform
            # If rotated, we rotate 90 deg. 
            # Rotation center? Usually rotate around (0,0) then translate.
            # SVG rotate(90) moves X->Y, Y->-X.
            # We need to shift it back into the bounding box.
            # Bounding Box is (x_pos, y_pos) of size (p_info['w'], p_info['h']).
            # Note: p_info['w'] is the PACKED width. If rotated, this is item['h'].
            
            # Transform string: 
            # translate(x, y) 
            # [if rot] rotate(-90) translate(-W, 0)? Or rotate(90) translate(0, -H)?
            # Let's say Original is W_o x H_o.
            # Rotated is H_o x W_o.
            # We place Rotated Rect at (x, y).
            # Inside, we have Original Rect.
            # transformation: `translate(x, y) rotate(90)`.
            #   (0,0) -> (x,y) -> (x, y).
            #   (W,0) -> (x+W, y) -> (x, y+W).
            #   (0,H) -> (x, y+H) -> (x-H, y). !! Goes negative X.
            # We need to shift X positive by H (the new width).
            # `translate(x + H_o, y) rotate(90)`?
            #   (0,0) -> (x+H, y) -> (x+H, y). Correct top-right of rotated? No.
            # Let's visualize: 
            #   Desired: Top-Left at (x, y).
            #   Rotate 90 deg clockwise? `transform="rotate(90)"`.
            #   (0,0)->(0,0). (W,0)->(0,W). (0,H)->(-H,0).
            #   Bounding box starts at -H.
            #   To put Top-Left at (0,0), translate(H, 0).
            #   So `translate(x + H_orig, y) rotate(90)`.
            
            tf = f'translate({f(x_pos)}, {f(y_pos)})'
            if rotated:
                # p_info['w'] is the width taken in sheet (which is H_orig).
                tf = f'translate({f(x_pos + part["h"])}, {f(y_pos)}) rotate(90)'
                
            # Process Files
            # If FRONT has Nested Hatch, we must also render Hatch relative to Front.
            # Hatch needs to rotate WITH Front.
            
            files_to_render = part['files'].copy()
            if cleat_data: # Just ensure we have data context
                if part.get('id') == "CLEAT_WALL": files_to_render = cleat_data['wall_cleat_files']
                if part.get('id') == "CLEAT_BOX": files_to_render = cleat_data['box_cleat_files']
            
            # Helper to extract body
            def extract_body(content, shift_margin=True):
                body_match = re.search(r'(?s)<svg[^>]*>(.*?)<\/svg>', content)
                if body_match:
                    body = body_match.group(1)
                    if shift_margin:
                        # Shift by -MARGIN to zero the content
                        return f'<g transform="translate(-{f(MARGIN_INCHES)}, -{f(MARGIN_INCHES)})">{body}</g>'
                    return body
                return ""

            for fname, content in files_to_render.items():
                if isinstance(content, str) and fname not in part['files']:
                     # It's a key from cleat_data mapping, content is filename?
                     # Fix logic: cleat_data['wall_cleat_files'] maps TYPE -> Filename.
                     # We need content from cleat_data['svg_contents'].
                     if cleat_data:
                         if content in cleat_data['svg_contents']:
                             content = cleat_data['svg_contents'][content]
                             
                if "VISUALIZATION" in fname: continue
                l_type = get_layer_type(fname)
                if not l_type: continue
                
                clean = l_type.split('_')[1]
                sub_id = f"{clean}_{part['id']}"
                
                body = extract_body(content)
                block = f'      <g id="{sub_id}" transform="{tf}">{body}</g>'
                layers[l_type].append(block)
                
            # Render NESTED HATCH if applicable
            if part.get('nested_hatch'):
                h_data = part['nested_hatch']
                # Hatch is centered in Front.
                # Center of Front (Original Coords): (Total_W/2, Total_H/2). (After Margin shift).
                # Center of Hatch (Original Coords): (Lid_W/2, Lid_H/2). (After Margin shift).
                # Hatch Offset relative to Front Origin: (Total_W - Lid_W)/2, (Total_H - Lid_H)/2.
                
                off_x = (part['w'] - h_data['lid_w']) / 2
                off_y = (part['h'] - h_data['lid_h']) / 2
                
                # Hatch Transform.
                # It is inside the Front Group. So it inherits Front's Rotation.
                # We simply translate it relative to Front's origin.
                # BUT, Hatch SVG has its own Margin/Origin.
                # extract_body shifts Hatch to (0,0).
                # So we translate by off_x, off_y.
                
                h_content = h_data['svg_content']
                h_body = extract_body(h_content)
                
                # Treat as CUT
                h_block = f'      <g id="CUTS_NESTED_HATCH" transform="{tf}"><g transform="translate({f(off_x)}, {f(off_y)})">{h_body}</g></g>'
                layers['01_CUTS'].append(h_block)

        # Output Layers
        for l_name in sorted(layers.keys()):
            if layers[l_name]:
                lines.append(f'    <g id="{l_name}">')
                lines.extend(layers[l_name])
                lines.append('    </g>')
                
        lines.append('  </g>')
        lines.append('</svg>')
        
        fname = f"MASTER_LAYOUT_v{version}_SHEET_{sheet_idx+1}.svg"
        if sheet_idx == 0: fname = f"MASTER_LAYOUT_v{version}.svg" # Keep primary name for sheet 1
        
        master_svgs[fname] = "\n".join(lines)
        
    # Return Primary Sheet (for compatibility) + write all sheets handling
    # The caller expects a single string or dict?
    # Original caller: `all_files[...master...] = master_svg`.
    # We should change caller to handle dict, but here we only return ONE string if strictly following signature.
    # Hack: We will return the Main Sheet as the string. 
    # BUT, we need to save the other sheets. 
    # Since we can't change the caller easily in `run_generation` without editing that block too...
    # We will Return the Main Sheet, AND write the others explicitly here? 
    # No, Side Effects are bad.
    # Better: Update `run_generation` to handle multiple returns or stash them.
    # Actually, we are rewriting `run_generation` logic? No, we replaced the function definition.
    # The Caller `run_generation` calls this.
    # Let's check `run_generation`.
    # `all_files[f"MASTER_LAYOUT_v{next_ver}.svg"] = master_svg`
    # If we return a Dict, `all_files[key] = Dict` -> Crash on write.
    
    # SOLUTION: Return the Content of Sheet 1.
    # BUT we need Sheet 2.
    # We will sneakily return a String that contains ALL SVGs concatenated? No, invalid SVG.
    # We must patch `run_generation` as well.
    # Let's assume we patch `run_generation` in the next step or same tool call? 
    # I can edit `run_generation` too.
    
    return master_svgs # Breaking change, must update caller.
...
class CarbideOptimizedApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        # Version
        tk.Label(container, text="v1.07", font=FONT_VERSION, bg=BG_COLOR, fg=TEXT_HINT).place(relx=1.0, rely=1.0, anchor="se")
