...
def create_shadowbox_assembly():
    """Create the complete Shadowbox assembly."""
    ...
    if len(top_verts) >= 3:
        parts['TOP'] = create_mesh_from_profile("Box_Top_Rail", top_verts, stock_thk, collection, 'Z')
    else:
        parts['TOP'] = create_simple_box("Box_Top_Rail", total_w, stock_thk, box_d, collection)

    # BOTTOM RAIL
    print("  - Bottom Rail")
    bot_verts = parse_svg_path(SVG_PATHS['BOTTOM_RAIL'])
    if len(bot_verts) >= 3:
        parts['BOTTOM'] = create_mesh_from_profile("Box_Bottom_Rail", bot_verts, stock_thk, collection, 'Z')
    else:
        parts['BOTTOM'] = create_simple_box("Box_Bottom_Rail", total_w, stock_thk, box_d, collection)

    # LEFT RAIL
    print("  - Left Rail")
    left_verts = parse_svg_path(SVG_PATHS['LEFT_RAIL'])
    if len(left_verts) >= 3:
        parts['LEFT'] = create_mesh_from_profile("Box_Left_Rail", left_verts, stock_thk, collection, 'Z')
        parts['LEFT'].rotation_euler = (0, math.radians(90), 0)
    else:
        parts['LEFT'] = create_simple_box("Box_Left_Rail", stock_thk, total_h, box_d, collection)

    # RIGHT RAIL
    print("  - Right Rail")
    right_verts = parse_svg_path(SVG_PATHS['RIGHT_RAIL'])
    if len(right_verts) >= 3:
        parts['RIGHT'] = create_mesh_from_profile("Box_Right_Rail", right_verts, stock_thk, collection, 'Z')
        parts['RIGHT'].rotation_euler = (0, math.radians(-90), 0)
    else:
        parts['RIGHT'] = create_simple_box("Box_Right_Rail", stock_thk, total_h, box_d, collection)

    # FRONT BEZEL
    print("  - Front Bezel")
    parts['FRONT'] = create_simple_box("Box_Front_Bezel", total_w, total_h, stock_thk, collection)

    # BACK PANEL
    print("  - Back Panel")
    parts['BACK'] = create_simple_box("Box_Back_Panel", total_w, total_h, stock_thk, collection)
