...
#
# Key Changes:
# - Bottom Hatch Logic Updated (User Requested):
#   - Fixed Scale Rabbet Width: 0.64" (Was previously derived from stock).
#   - Hatch Opening Shrinks to accommodate wider rabbet (Lid Outer Size fixed? No, logic implies checking constraints).
#   - Actually: User directed "Hatch Rabbet widths = fixed .64"". "Opening will shrink".
#   - Added 4 Corner Holes:
#       - Lid: 0.2" Diameter.
#       - Back Panel Rabbet: 0.28125" Diameter (Through/Deep).
#       - Concentric placement.
#       - Clearance: 0.075" form edges -> Total Width = 0.075+0.28125+0.075 = 0.43125".
#       - Wait, User said "rabbet depth remains...".
#       - User said "rabbet widths = fixed .64"".
#       - 0.64" is > 0.43125", so plenty of clearance (0.64 - 0.28125)/2 = 0.179" per side.
#   - Version Bump to v1.12.
#
"""
...
def generate_back_panel_parts(version):
    ...
    if CONFIG.get('HATCH_ENABLED', False):
        ...
        # NEW Constraint: Rabbet Width is FIXED at 0.64 inches
        # "Inside hatch cover becomes 96% of its previously generated size" -> This was a loose guideline, 
        # but specific instruction 1 overrides: "Mating Hatch Rabbet widths = fixed .64"".
        # Specific instruction 2: "Opening will shrink to accomodate the wider rabbet".
        
        # Previously: Lid = Open + 2*Flange. 
        # If we keep Lid Size constant (as implied by "Opening will shrink"), we calculate Lid relative to something?
        # Re-reading: "Note that this will NOT involve overriding the size of the hatch hole that the User specifies"
        # BUT User Request 2 says: "Option A, Opening will shrink to accomodate the wider rabbet".
        # This implies the OUTER border of the hatch (The shelf on the back panel) stays where it was?
        # Or does the Lid stay the same size?
        # Let's assume the "Hatch Lid" Outer Dimensions are what we calculated before (Open + Old_Flange)
        # and now we are applying New_Flange (0.64) inwards, thus shrinking the opening.
        
        # Old Flange Logic (for reference "Pre-Shrink" state):
        old_glue_gap = convert_to_inches(CONFIG.get('FIT_TOLERANCE', 0.254))
        old_flange_w = (stock_thk / 2.0) - old_glue_gap
        
        # Hypothetical "Original" Lid Size based on User PCT
        ...
        # Opening = Lid - 2 * Flange
        # We start with the Orig Lid Size and apply new flange.
        final_open_w = orig_lid_w - (2 * new_flange_w)
        final_open_h = orig_lid_h - (2 * new_flange_w)
        
        # Final Lid Size (Should match Orig Lid Size for the "Cover" to stay same footprint, just different cutout?)
        # User said "inside hatch cover becomes 96%". 
        # If we kept Lid fixed, and increased flange, the "Inside Cover" (the part that fits in hole?) shrinks.
        # Yes, "Opening will shrink".
        ...
        # Shelf extends `new_flange_w` outwards from Opening.
        # Check math: Open + 2*Flange = Orig_Lid. Correct.
        ...
        # NEW: Holes in Back Panel Rabbet (Through Holes for Inserts/Bolts?)
        # User: "0.28125" holes... centered in the corners [of] this new wider rabbet"
        # Hole Center is exactly half-way through the flange width.
        hole_offset = new_flange_w / 2.0
        
        # Panel Hole Diameter = 0.28125
        panel_hole_r = 0.28125 / 2.0
        
        # Coordinates relative to Shelf/Pocket Top-Left (shelf_x, shelf_y)
        # Top-Left Corner
        hx_tl = shelf_x + hole_offset
        hy_tl = shelf_y + hole_offset
        cut_elements.append(create_circle(hx_tl, hy_tl, panel_hole_r, COLOR_HOLES))
        
        # Top-Right Corner
        hx_tr = shelf_x + shelf_w - hole_offset
        hy_tr = shelf_y + hole_offset
        cut_elements.append(create_circle(hx_tr, hy_tr, panel_hole_r, COLOR_HOLES))
        
        # Bot-Left Corner
        hx_bl = shelf_x + hole_offset
        hy_bl = shelf_y + shelf_h - hole_offset
        cut_elements.append(create_circle(hx_bl, hy_bl, panel_hole_r, COLOR_HOLES))
        
        # Bot-Right Corner
        hx_br = shelf_x + shelf_w - hole_offset
        hy_br = shelf_y + shelf_h - hole_offset
        ...
        # NEW: Holes in Lid (Alignment Holes)
        # User: "four 0.2" Holes... match perfectly with matching centers"
        lid_hole_r = 0.2 / 2.0
        
        # Same Offset logic (Center of Flange)
        # Top-Left
...
class CarbideOptimizedApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        tk.Label(container, text="v1.11", font=FONT_VERSION, bg=BG_COLOR, fg=TEXT_HINT).place(relx=1.0, rely=1.0, anchor="se")
